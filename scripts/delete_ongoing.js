#!/usr/bin/env node
/*
  Safe helper to find and optionally delete 'ongoing' import documents.
  ...
  Usage:
    ...
    3. Run: node scripts/delete_ongoing.js
  ...
  The script will:
    ...
    - Delete in batches and report progress
*/
...
async function main() {
  ...
  const confirm = (await prompt(`Delete these ${matches.length} document(s) from collection '${collectionName}'? Type 'yes' to confirm: `)).trim();
  ...
  for (let i = 0; i < matches.length; i += BATCH_SIZE) {
    ...
    console.log(`Deleted ${deleted}/${matches.length} documents...`);
  }
