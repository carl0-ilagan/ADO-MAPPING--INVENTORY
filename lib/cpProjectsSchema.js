...
export const CP_PROJECTS_SCHEMA = {
  ...
  worksheet_no: 'string',                // Original row number from the workbook list
  ...
  workflow_status: 'string',             // Raw worksheet STATUS value (e.g. Ongoing FPIC process)
 ...
 */
// Firestore Timestamp valid seconds range: year 0001 to 9999
const FIRESTORE_MIN_SECONDS = -62135596800;
const FIRESTORE_MAX_SECONDS = 253402300799;

function isFirestoreSafeDate(d) {
  if (!(d instanceof Date) || isNaN(d.getTime())) return false;
  const secs = Math.floor(d.getTime() / 1000);
  return secs >= FIRESTORE_MIN_SECONDS && secs <= FIRESTORE_MAX_SECONDS;
}
...
export function parseDate(dateValue) {
  ...
  try {
    ...
    if (typeof dateValue === 'object' && dateValue !== null && typeof dateValue.toDate === 'function') {
      ...
      return isFirestoreSafeDate(d) ? d : null;
    }
    
    // If it's a Date object
    if (dateValue instanceof Date) {
      return isFirestoreSafeDate(dateValue) ? dateValue : null;
    }
    
    // If it's a number - could be a plain year, Excel serial date, or ms unix timestamp
    if (typeof dateValue === 'number') {
      // Plain 4-digit year (e.g. 2010, 2018) — store as Jan 1 of that year
      if (Number.isInteger(dateValue) && dateValue >= 1900 && dateValue <= 2100) {
        const d = new Date(Date.UTC(dateValue, 0, 1));
        return isFirestoreSafeDate(d) ? d : null;
      }
      // Small number = Excel serial date (days since Jan 0, 1900)
      // Excel serial 1 = Jan 1, 1900; ~45000 = year 2023
      // Valid Excel serial range for 1900–2100 is roughly 1–73050
      ...
      // Medium number = Unix seconds timestamp
    ...
    if (typeof dateValue === 'string') {
      ...
      if (isFirestoreSafeDate(date)) return date;
      // Try numeric string — plain year or Excel serial
      const asNum = Number(trimmed);
      if (!isNaN(asNum) && asNum > 0) {
        // Plain 4-digit year
        if (Number.isInteger(asNum) && asNum >= 1900 && asNum <= 2100) {
          const d = new Date(Date.UTC(asNum, 0, 1));
          return isFirestoreSafeDate(d) ? d : null;
        }
        if (asNum > 0 && asNum < 200000) {
          // Excel serial
...
export function buildCPProjectDocument(rowData, fieldMap, sheetName = '', defaultStatus = 'Ongoing') {
  ...
  if (sheetName) {
    doc.source_sheet = String(sheetName).trim();
  }
  ...
  Object.entries(fieldMap).forEach(([fieldName, columnIndex]) => {
    ...
    if (value !== null && value !== undefined && value !== '') {
      ...
      } else if (fieldName === 'has_ongoing_fpic') {
        const raw = String(value).trim();
        doc[fieldName] = raw;
        // Some workbooks place community names in the "for CP with ongoing FPIC" column.
        // Preserve it as affected_icc as well so Pending table column displays correctly.
        if (!doc.affected_icc || (Array.isArray(doc.affected_icc) && doc.affected_icc.length === 0)) {
          const looksLikeFlag = /^(yes|no|y|n|true|false|1|0|ongoing|none|n\/a|na|-)$/i.test(raw);
          if (raw && !looksLikeFlag) {
            doc.affected_icc = raw.split(',').map(v => v.trim()).filter(Boolean);
          }
        }
      } else if (fieldName === 'status') {
        doc.workflow_status = String(value).trim();
      } else {
  ...
  // The selected import tab is authoritative for the master Pending/Approved/Ongoing status.
  // Preserve the worksheet STATUS text separately in workflow_status.
  doc.status = normalizeStatus(defaultStatus || doc.status || 'Pending');
...
export function detectRegionFromSheetName(sheetName) {
  ...
  // Region 6-7 / 6&7 / 67 (with or without separator)
...
export function validateCPProject(doc) {
  ...
  // At least one core content field must be non-empty — this filters out blank Excel rows
  // that only got a region value from the sheet name
  const hasContent =
    (doc.project_name && String(doc.project_name).trim() !== '') ||
    (doc.proponent && String(doc.proponent).trim() !== '') ||
    (doc.control_number && String(doc.control_number).trim() !== '');

  if (!hasContent) {
    errors.push('Row has no meaningful data (project_name, proponent, and control_number are all empty)');
  }
