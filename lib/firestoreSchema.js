...
export function buildFirestoreDocument(excelRow, fieldMap, sheetName, batchId = '') {
  ...
  if (Math.random() < 0.05) { // Log ~5% of records
    console.log('📝 Sample Document:', {
      sheet: sheetName,
      row: excelRow.slice(0, 12),  // Show first 12 columns
      fieldMap,
      extracted: {
        region: doc.region,
        control_number: doc.control_number,
        survey_number: doc.survey_number,
        applicant_proponent: doc.applicant_proponent,
        name_of_project: doc.name_of_project,
        location: doc.location,
        province: doc.province,
        cadt_status: doc.cadt_status,
        icc: doc.icc,
