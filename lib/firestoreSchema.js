...
export const HEADER_MAPPINGS = {
  ...
  // Ongoing workflow / FPIC process columns (summary fields)
  ...
  issuance_resolution_of_consent: [
    ...
    'issuance resolution of consent',
  ],
  review_by_rrt: [
...
export function mapHeadersToFields(excelHeaders) {
  ...
  // For any fields still unmapped, try a looser, word-overlap based mapping.
  // This helps when users upload CSVs with slightly different header phrasing.
  Object.keys(HEADER_MAPPINGS).forEach((field) => {
    if (fieldMap[field] !== undefined) return; // already mapped

    const targetWords = field
      .replace(/_/g, ' ')
      .split(/\s+/)
      .filter(Boolean);
    ...
    normalizedHeaders.forEach((hdr, idx) => {
      ...
      if (hdrWords.length === 0) return;

      // compute word-overlap score (how many target words appear in header)
      const overlap = targetWords.filter(w => hdrWords.some(h => h.includes(w) || w.includes(h))).length;
      const score = overlap / targetWords.length;

      // prefer longer headers (more specific) when tie
      const lengthBonus = Math.min(0.2, hdrWords.length * 0.02);
    ...
    if (bestIdx !== -1) {
      fieldMap[field] = bestIdx;
      console.log(`  ⚡ aggressive-map ${field} → column ${bestIdx} (${excelHeaders[bestIdx]}) score=${bestScore.toFixed(2)}`);
    }
  });
  ...
  switch (fieldType) {
    ...
    case 'timestamp': {
      ...
      if (!isNaN(d.getTime())) {
        ...
        if (secs >= -62135596800 && secs <= 253402300799) return d;
      }
      return null;
    }
 ...
 */
export function buildFirestoreDocument(excelRow, fieldMap, sheetName, batchId = '', headerRowOriginal = []) {
  const doc = {
  ...
  // Preserve original CSV values under `raw_fields` using sanitized header keys.
  // This allows the UI to display the exact uploaded data when canonical
  // mapping fails or headers are non-standard.
  try {
    doc.raw_fields = {};
    for (let i = 0; i < (headerRowOriginal || []).length; i += 1) {
      const rawKey = String(headerRowOriginal[i] || `col_${i}`);
      const safeKey = rawKey.trim().replace(/\s+/g, '_').replace(/[^A-Za-z0-9_]/g, '_').toLowerCase();
      const value = excelRow && excelRow[i] !== undefined && excelRow[i] !== null ? String(excelRow[i]).trim() : '';
      doc.raw_fields[safeKey] = value;
    }
  } catch (e) {
    // ignore failures preserving raw fields
  }
  ...
  // Backwards-compatibility aliases for ongoing summary fields
  doc.issuanceOfWorkOrder = doc.issuance_of_work_order || doc.work_order || '';
  doc.preFBIConference = doc.pre_fbi_conference || '';
  doc.conductOfFBI = doc.conduct_of_fbi || '';
  doc.reviewOfFBIReport = doc.review_of_fbi_report || '';
  doc.preFPICConference = doc.pre_fpic_conference || '';
  doc.firstCommunityAssembly = doc.first_community_assembly || doc['1st_community_assembly'] || '';
  doc.secondCommunityAssembly = doc.second_community_assembly || doc['2nd_community_assembly'] || '';
  doc.consensusBuildingDecision = doc.consensus_building_decision || '';
  doc.moaValidationRatificationSigning = doc.moa_validation_ratification_signing || '';
  doc.issuanceResolutionOfConsent = doc.issuance_resolution_of_consent || '';
  doc.reviewByRRT = doc.review_by_rrt || '';
  doc.reviewByADOorLAO = doc.review_by_ado_or_lao || '';
  doc.forComplianceOfFPICTeam = doc.for_compliance_of_fpic_team || '';
  doc.cebDeliberation = doc.ceb_deliberation || '';
