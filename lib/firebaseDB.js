    ...
    querySnapshot.forEach((docSnap) => {
      ...
      try {
        if (mappings.length < 5) {
          ...
          console.debug(`firebaseDB: first 5 docs from ${collectionName}: id=${docSnap.id}, _pending=${data._pending}, _ongoing=${data._ongoing}, has_raw_fields=${!!data.raw_fields}`);
        }
      } catch (e) {
      ...
      if (data && typeof data === 'object') {
        ...
        if (typeof data._pending !== 'undefined') preserved._pending = data._pending;
      }
      ...
      try {
        if (data && typeof data === 'object') {
          if (data.raw_fields && typeof data.raw_fields === 'object') {
            ...
            const meta = new Set(['createdAt','updatedAt','userId','sheet','_ongoing','_pending','importCollection','source_sheet','import_batch_id','imported_at']);
