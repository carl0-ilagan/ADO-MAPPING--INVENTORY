...
export const importMappings = async ({ preparedDocs = [], rawRecords = [], mode = 'add', collectionName = 'cp_projects', userId = null, idsToDelete = [], onProgress = () => {}, forceOngoing = false }) => {
  ...
  // Register the import collection only for non-primary collections.
  try {
    if (collectionName && collectionName !== 'cp_projects') {
