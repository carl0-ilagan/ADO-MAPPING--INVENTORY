...
// Batch configuration tuned for free-tier safety.
// Smaller batches + longer gaps reduce burst pressure on Firestore.
...
const commitWithRetries = async (batch, maxRetries = 5) => {
  ...
  while (true) {
    try {
      ...
      const isQuotaExceeded = msg.includes('resource-exhausted') || code.includes('resource-exhausted');
      ...
      // Quota exceeded is typically not transient within the same run/day.
      // Stop immediately and surface a clear action to the user.
      if (isQuotaExceeded) {
        const quotaErr = new Error('Firestore quota exceeded. Please wait for quota reset or enable billing, then retry import.');
        quotaErr.code = err?.code || 'resource-exhausted';
        quotaErr.cause = err;
        throw quotaErr;
      }
  ...
  for (const sheet of excelSheets) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        // Make Summary totals authoritative by accepting up to the cap per region.
        if (isPendingCapMode) {
          if (acceptedForSheet >= capForSheet) {
            continue;
          }
          const hasAnyRowData = Array.isArray(row)
