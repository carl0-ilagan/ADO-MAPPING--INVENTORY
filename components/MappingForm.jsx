...
// SearchableSelect: button trigger, floating panel, supports single and multi (with Sets)
function SearchableSelect({ options = [], selected = null, onChange = () => {}, placeholder = 'Select', multi = false, disabled = false, summaryLabels = { singular: 'item', plural: 'items' } }) {
  ...
  return (
    <div className="relative">
      <button ref={triggerRef} type="button" disabled={disabled} onClick={() => !disabled && setOpen(v => !v)} className={cn('w-full flex items-center justify-between px-4 py-3 text-sm border-2 rounded-lg bg-white/80 hover:border-[#F2C94C]/40 transition', disabled ? 'opacity-60 cursor-not-allowed' : '')}>
...
export default function MappingForm({ isModal = false, onBack = () => {} }) {
  ...
  // Render helpers per requirements
  ...
  // when parent changes clear dependents
  useEffect(() => {
  ...
  // keep barangays in sync when municipalities change
  useEffect(() => {
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {savedSelections.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-6 text-[#0A2D55]/50">No saved data yet.</td></tr>
                  ) : (
...
// also provide a named export for consumers that import { MappingForm }
export { MappingForm };
